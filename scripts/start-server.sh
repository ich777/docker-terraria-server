...
if [ "${ENABLE_TML64}" == "true" ]; then
    ...
    if [ -z "$CUR_MOD64_V" ]; then
        ...
        rm "${SERVER_DIR}/tModLoader64BitServer"
...
if [ "${ENABLE_TML64}" == "true" ]; then
    ...
    if [ ! -f "${SERVER_DIR}/tModLoader64BitServer.bin.x86" ]; then
        echo "---Applying x86 MonoKickstart to TML64---"
        cp ${SERVER_DIR}/tModLoaderServer.bin.x86 ${SERVER_DIR}/tModLoader64BitServer.bin.x86
    fi
    if [ ! -f "${SERVER_DIR}/tModLoader64BitServer.bin.x86_64" ]; then
        echo "---Applying x86_64 MonoKickstart to TML64---"
        cp ${SERVER_DIR}/tModLoaderServer.bin.x86_64 ${SERVER_DIR}/tModLoader64BitServer.bin.x86_64
    fi
fi
echo "---Server ready---"
